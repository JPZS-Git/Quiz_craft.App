...
----------------------------------------
- SUFFIX: sufixo do repositório (ex.: `Answers`). Será usado para formar o nome da classe abstrata: `<SUFFIX>Repository`.
- ENTITY: nome da entidade/model (ex.: `Answer`) — usado nos tipos de retorno.
...
2. Confirmar o caminho do arquivo que declara a classe (ex.: `lib/features/answers/domain/entities/answer.dart`).
3. Verificar que a classe exporta o símbolo correto (ex.: `class Answer`).
4. Determinar o nome em minúsculas/plural para formar caminhos (ex.: `answer` → `answers`).
...
4. Preserve tipos exatos conforme `ENTITY` (por exemplo `Future<List<Answer>>`).
...
--------------------------------
1. Determine `entity_em_minusculas` a partir de `ENTITY` (ex.: `Answer` → `answer`). Recomende também a forma plural/coleção usada no projeto (ex.: `answers`) e use `DEST_DIR` como `lib/features/<entity_em_minusculas_plural>/domain/repositories/` por padrão.
...
*/
```
2. Crie o arquivo em `DEST_DIR` com o nome `<entity_em_minusculas>_repository.dart` (por exemplo `answers_repository.dart`).
