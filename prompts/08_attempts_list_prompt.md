`````markdown
````markdown
## Prompt: Gerar listagem de attempts (attempts)

Parâmetros (defina estes valores no início do prompt antes de executar)
---------------------------------------------------------------
- ENTITY_SINGULAR: Attempt    # nome singular legível (ex.: 'Attempt')
- ENTITY_PLURAL: attempts     # nome plural legível em minúsculas (ex.: 'attempts')
- DTO_CLASS: AttemptDto      # nome da classe/DTO usada no projeto
- FEATURE_FOLDER: attempts   # pasta/feature onde o código vive
...
- SORT_BY_DEFAULT: startedAt # campo de ordenação padrão
- INCLUDE_HINT: quiz,user    # relacionamentos opcionais sugeridos

OBS: Substitua os tokens acima antes de enviar o prompt ao agente ou forneça esses valores na invocação.

Contexto
---
Você é um agente que ajuda a produzir tanto a especificação de dados quanto, quando solicitado, código Flutter/Dart que implemente a listagem de tentativas (`attempts`) seguindo as convenções do repositório. O prompt abaixo está parametrizado para permitir gerar:

- uma especificação/contract (JSON) para a API/DAO que fornece a listagem;
- e opcionalmente, um widget Flutter/Dart muito semelhante ao padrão do repositório, com integração ao DAO local da feature.

Objetivo
---
Gerar (conforme modo solicitado na invocação) uma das seguintes saídas:

1. Especificação e exemplo de JSON (contrato de dados) para a listagem paginada e filtrável de `Attempt`;
2. Código Flutter/Dart que implemente a página/listagem de `attempts` (widget), integrando com o DAO local da feature, e obedecendo às convenções do projeto (idioma, nomes, patterns de persistência e handlers de UI).

Entradas esperadas (inputs)
---
- filters: objeto opcional com critérios de busca (por exemplo: {"q": "usuario", "quizId": "..."})
- page: número da página (inteiro, default 1)
- pageSize: itens por página (inteiro, default 20)
- sortBy: campo para ordenação (por exemplo: "startedAt", "score")
- sortDir: direção da ordenação ("asc" ou "desc")
- include: lista opcional de relacionamentos a incluir (por exemplo: ["quiz","user"]) — o agente deve explicar resoluções possíveis.

Se o modo de execução pedir geração de código, o agente deve aceitar parâmetros adicionais (passe-os como tokens no topo):

- DAO_CLASS_HINT: AttemptsLocalDaoSharedPrefs  # sugestão de nome do DAO local a ser importado
...
- Campos sensíveis devem ser mascarados na saída de exemplo (por exemplo: emails parcialmente ocultos) e, no código gerado, aplicar máscaras quando exibir tais campos. Para `Attempt` normalmente não há dados pessoais sensíveis além do `user.email`, então trate esse campo com cuidado quando exibido.
...
- Permissões: o código gerado e a especificação devem mencionar que a listagem respeita permissões/escopo do usuário (exibir apenas attempts que o usuário tem acesso).
...
- Renderização resumida de cada `Attempt` (ex.: `quizId` ou quiz.title quando disponível, `user` (mascarado), `score`, `startedAt`).
- Renderização condicional de detalhes quando `include` for solicitado — implementar como expansão inline opcional (opcional para listing-only), mas o widget deve sempre suportar a presença dos campos opcionais no DTO.
- Integração com DAO local da feature: identificar e importar o DAO (por exemplo `AttemptsLocalDaoSharedPrefs`) e usar os métodos de leitura (`listAll`) — operações de escrita/remoção/edição devem ser mantidas fora deste arquivo e implementadas separadamente.

Observação: funcionalidades adicionais (visualizar detalhes, exportar CSV, ordenação por coluna) devem ser implementadas em arquivos/pedidos separados.

Contrato de DTO / Campos esperados
---
O agente deve mapear o DTO aos campos usados no código existente. Campos normalmente presentes no projeto e em `AttemptDto`:
...
- totalCount: int
- score: number (double)
- startedAt / finishedAt: ISO8601 datetimes
- metadata/opcional: map (opcional)
...
- data: lista de objetos de attempt, cada um contendo ao menos os campos abaixo

Campos de cada attempt (exemplo mínimo)
---
- id: string (UUID)
- quizId: string
- userId: string (mascarado na exibição, ex.: "joao***@dominio.com")
- correctCount: integer
- totalCount: integer
- score: number
- startedAt: ISO8601 datetime
- finishedAt: ISO8601 datetime
...
1. O agente gera um widget Flutter/Dart que implementa a página de listagem com comportamento compatível (loading, list, details expansion quando aplicável).
...
- Sugira campos opcionais que podem ser incluídos se solicitado (ex.: duration, attemptMetadata) e como isso impacta performance.
...
- adicione apenas os arquivos mínimos necessários (ex.: `attempts_page.dart`, `attempt_list_item.dart`) e siga as convenções de nomes e pastas;
...
Resumo
---
Este prompt agora está parametrizável e adaptado à feature `attempts`. Configure os tokens no topo (`ENTITY_SINGULAR`, `DTO_CLASS`, `FEATURE_FOLDER`, `DAO_CLASS_HINT`, etc.) antes de executar o prompt com o agente.
