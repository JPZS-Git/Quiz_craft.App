````markdown
## Prompt: Gerar listagem de peguntas (questions)

Parâmetros (defina estes valores no início do prompt antes de executar)
---------------------------------------------------------------
- ENTITY_SINGULAR: Pergunta   # nome singular legível (ex.: 'Fornecedor')
- ENTITY_PLURAL: perguntas    # nome plural legível em minúsculas (ex.: 'fornecedores')
- DTO_CLASS: QuestionDto        # nome da classe/DTO usada no projeto (ex.: 'ProviderDto')
- FEATURE_FOLDER: questions      # pasta/feature onde o código vive (ex.: 'providers')
...
Contexto
---
Você é um agente que ajuda a produzir tanto a especificação de dados quanto, quando solicitado, código Flutter/Dart que implemente a listagem de Perguntas (widget/fluxo) seguindo as convenções do repositório. O prompt abaixo está parametrizado para permitir gerar:
...
---
O agente deve mapear o DTO aos campos usados no código existente. Campos normalmente presentes no projeto e no `question_entity.dart`:
